...
#Attributes:
 ...
 crm <- read.csv("CRM.csv", stringsAsFactors = T) #will autoencode the text attributes to factors
 ...
 write.csv(file="mydata.csv", crm, row.names = F)
 ...
 crm <- read.csv("mydata.csv", stringsAsFactors = T)
 
 str(crm)
 
 #let's deal with these other factors: F1
 crm$Education <- factor(crm$Education, levels = c(1,2,3,4,5), labels=c("BC", "C", "UG", "MSc", "PhD"))
 crm$EnvironmentSatisfaction <- factor(crm$EnvironmentSatisfaction, levels = c(1:4), labels=c("Low", "Medium", "High", "v. High"))
 crm$JobInvolvement <- factor(crm$JobInvolvement, levels = c(1:4), labels=c("Low", "Medium", "High", "v. High"))
 crm$JobLevel <- factor(crm$JobLevel) #insufficient information to do more
 crm$JobSatisfaction <- factor(crm$JobSatisfaction, levels = c(1:4), labels=c("Low", "Medium", "High", "v. High"))
 crm$PerformanceRating <- factor(crm$PerformanceRating, levels = c(1:4), labels=c("Low", "Good", "Excellent", "Outstanding"))
 crm$RelationshipSatisfaction <- factor(crm$RelationshipSatisfaction, levels = c(1:4), labels=c("Low", "Medium", "High", "v. High"))
 crm$StockOptionLevel <- factor(crm$StockOptionLevel) #don't have more information
 crm$WorkLifeBalance <- factor(crm$WorkLifeBalance, levels = c(1:4), labels=c("Bad", "Good", "Better", "Best"))
 ...
 crm$PerformanceRating <- factor(crm$PerformanceRating) 
 ...
 crm <- crm[,-9] #EmployeeCount -- always 1
 crm <- crm[, -19] #over 18 -- always Y; strictly speaking we should confirm this, but under time pressure it's safe to assume this varibale isn't needed
 
 #mising values:
 sapply(crm,function(x) sum(is.na(x)))
 
 #outliers
 boxplot(subset(crm, select=c(18,20,21:22))) #nothing too obvious here
 boxplot(subset(crm, select=c(26:28))) #May be something in years at company
 boxplot(crm$DailyRate)
 boxplot(crm$HourlyRate)
 boxplot(crm$MonthlyIncome) #possibly some here too
 boxplot(crm$MonthlyRate)
 
 #i prefer the attrition factor as 0/1 for convenience 
 crm$Attrition <- factor(crm$Attrition, labels=c(0,1), levels=c("No", "Yes"))
 
 #F2: class balance:
 table(crm$Attrition) #fairly imbalanced
 
 #F3: train and test
 #2 options here stratified, or under/over sampling
 library(caret)
 sample <- createDataPartition(crm$Attrition, p = .75, list = FALSE) 
 train <- crm[sample, ]
 test <- crm[-sample, ]
 ...
 str(subset(crm, select=c(1,4,6,10,16:19,23,24,26:29)))
 #now correlation test
 cor(subset(crm, select=c(1,4,6,10,16:19,23,24,26:29)))
 
 #End of Question
 ##########################################
 
 #Start of Question: B2
 boxplot(crm$Age ~ crm$Attrition)
 spineplot(crm$JobSatisfaction, crm$Attrition) 
 ...
 spineplot(crm$MaritalStatus, crm$Attrition) #single people seem to leave more often
 boxplot(crm$DistanceFromHome ~ crm$MaritalStatus) #surprisingly there doesn't appear to be much difference in commute distance by martial status
 
 #End of Question
 ##########################################
 
 #Start of Question: B4
 plot(crm$Age, crm$MonthlyRate, main = "Scatter plot of Age vs. Monthly Rate")
 plot(crm$Age, crm$DailyRate, main = "Scatter plot of Age vs. Daily Rate")
 plot(crm$Age, crm$HourlyRate, main = "Scatter plot of Age vs. Hourly Rate")
 ...
 (counts <- table(crm$Attrition, crm$JobSatisfaction))
 ...
 crm_n <- subset(crm, select=c(1,4,6,10,16:19,23,24,26:29)) #get the numerical for normalisation -- kNN also doesn't support levelled factors either
 crm_n <- as.data.frame(lapply(crm_n, normalize)) #normalise
 summary(crm_n) #all our numericals are normalised, our categoricals are untouched
 
 #re make train and test note we can retain the original distribution if we choose to
 train_n <- crm_n[sample, ]
 test_n <- crm_n[-sample, ]
 ...
 dim(crm_n)
 crm_c <- subset(crm, select=c(2,3,5,7:9,11:15,20:22,25)) #isolate the categoricals
 crm_n <- cbind(crm_n, crm_c) #put them back together if you want to include the categoricals in a later question
 ...
 crm_c <- subset(crm, select=c(2,3,5,7:9,11:15,20:22,25)) #isolate the categoricals
 crm_k <- cbind(crm_n, crm_c) #put them back together 
 
 kmeansClusters <- list()
 kmeansScores <- c()
 for (i in 2:10) {
   clusters <- kmeans(crm_n, i)
 ...
 for (i in 2:10) {
   clusters <- kmeans(crm_n, i)
   name <- paste0("kmeans", i)
   dbindex <- index.DB(crm_n, clusters$cluster, centrotypes="centroids")
 ...
 #let's try plotting
 library(fpc)
 plotcluster(crm_n, kmeansClusters[["kmeans2"]]$cluster)
 plotcluster(crm_n, kmeansClusters[["kmeans4"]]$cluster)
 plotcluster(crm_n, kmeansClusters[["kmeans5"]]$cluster)
 plotcluster(crm_n, kmeansClusters[["kmeans10"]]$cluster)
 ...
 gower_dist <- daisy(crm_k, metric = "gower", type = list(logratio = 3))
 ...
 crm_old <- crm #don't want to lose it and do all the preprocessing again!
 
 #copy and paste relevent lines:
 crm <- hrdata[order(runif(600)), ]
 crm <- crm[-10]
 write.csv(file="myFulldata.csv", crm, row.names = F)
 crm <- read.csv("myFulldata.csv", stringsAsFactors = T)
 crm$Education <- factor(crm$Education, levels = c(1,2,3,4,5), labels=c("BC", "C", "UG", "MSc", "PhD"))
 crm$EnvironmentSatisfaction <- factor(crm$EnvironmentSatisfaction, levels = c(1:4), labels=c("Low", "Medium", "High", "v. High"))
 crm$JobInvolvement <- factor(crm$JobInvolvement, levels = c(1:4), labels=c("Low", "Medium", "High", "v. High"))
 crm$JobLevel <- factor(crm$JobLevel) #insufficient information to do more
 ...
 crm <- crm[,-9] #employees
 crm <- crm[,-20] #over18
 crm <- crm[,-24] #standard hours -- one i got back, but it's always 80, so has no value
 ...
 pcs <- prcomp( ~ ., data=crm_n) #PCA on categoricals is hard, so let's stick to the numerical attributes
 ...
 for (i in 1:1000) { #1000 samples of the b1 benchmark -- not stratified to get a non-zero sd; if this is a stratified sample the SD will likely be 0
   test_i <- crm[order(runif(sampleRate)), ]
 ...
 train_c <- crm_c[sample, ] #build the training categoricals 
 ...
 test_c <- crm_c[-sample, ] #build the training categoricals 
 test_n <- crm_n[-sample, ]
 ...
 #copy and paste from up top, but first backup crm
 
 crm_old <- crm
 crm <- read.csv("ADM CA 1 Data.csv", stringsAsFactors = T)
 crm <- crm[-10]
 crm$Education <- factor(crm$Education, levels = c(1,2,3,4,5), labels=c("BC", "C", "UG", "MSc", "PhD"))
 crm$EnvironmentSatisfaction <- factor(crm$EnvironmentSatisfaction, levels = c(1:4), labels=c("Low", "Medium", "High", "v. High"))
 crm$JobInvolvement <- factor(crm$JobInvolvement, levels = c(1:4), labels=c("Low", "Medium", "High", "v. High"))
 ...
 crm <- crm[,-9] #employees
 crm <- crm[,-20] #over18
 crm <- crm[,-24] #standard hours 
 
 crm$Attrition <- factor(crm$Attrition, labels=c(0,1), levels=c("No", "Yes"))
 
 #ok now we are good to go.
 #redo train and test
 sampleFull <- createDataPartition(crm$Attrition, p = .75, list = FALSE) 
 trainFull <- crm[sampleFull, ]
 testFull <- crm[-sampleFull, ]
